...
def resize_after_crop(img_crop_list, rat=0.9):
...
def mapping_after_resize(img_resize_list):
    ...
    temp = np.zeros((600, 400));
    temp[151:451, 101:301] = -1;
    img_map.append(temp);
...
def init_position():
    ...
    # 배경 이미지의 중앙 인덱스
    c_x, c_y = round(0.5 * 400), round(0.5 * 600);
...
def neighs(cur_state, more):

    ''' 현재 상태에서 10개의 이미지 중, 1위와 10위를 제외한 이미지의 좌표 list 중 하나의 좌표만을 좌우로는 +-10, 상하로는 +-5 움직인
        좌표들을 반환하는 함수 \n
        input : cur_state/ 현재 상태의 index list
                more/ Neighs를 사용할 함수(decision_position)에서  False로 미리 지정하고 시작하여 False인 경우, 좌우로 +-10한 list만,
    ...
    for i in range(1,len(cur_state)-1):
        ...
        temp1[i] = (cur_state[i][0] + 10 , cur_state[i][1]);
        temp2[i] = (cur_state[i][0] - 10 , cur_state[i][1]);
        neighs.append(temp1);
        neighs.append(temp2);
    ...
    if more is True:
        for i in range(1, len(cur_state) - 1):
            temp1 = cur_state.copy();
            temp2 = cur_state.copy();
            temp1[i] = (cur_state[i][0] , cur_state[i][1] + 5 );
            temp2[i] = (cur_state[i][0] , cur_state[i][1] - 5 );
            neighs.append(temp1);
            neighs.append(temp2);
...
def find_bestneighbor(img_map_list, neighs):
    ...
    input : img_map_list / 매핑된 image list(배경 사이즈로 패딩되기 전)
            neighs / cur_state의 Neighs output인 이웃 list \n
    ...
    for i in range(len(neighs)):
        ...
        try:
            img_paddingmap_list=padding_position(img_map_list, neighs[i]);
    ...
    best_neigh=neighs[neigh_score.index(max(neigh_score))];

    return best_neigh, max(neigh_score)

def decision_position(img_map_list,trace=False,more=False,maxRounds=50):
    ...
    if len(img_map_list)!=11:
        ...
        cur_index = [(init_index[i][0],init_index[i][1]-cor) for i in range(len(init_index))];
    ...
    while count < maxRounds:
        ...
        neighs_list=neighs(cur_index,more);
        best_neigh, next_value = find_bestneighbor(img_map_list,neighs_list);
        ...
        # 현재 상태가 이웃 상태들 중 가장 높은 평가 점수를 가지는 상태보다 더 높은 평가 점수를 가진다면
...
# def main():
#     ''' 배치가 잘 되는 지 확인하기 위해서 test를 위해 따로 작성한 함수로 실제 메인이 될 파일과 무관한 함수'''


#     # 이미지 불러오기 PART
#     image_list=[];

#     clf=make_classifier('haarcascade_frontalface_alt.xml');
...
#     for j in q:
#         if '.png' in j:
#             temp = cv2.imread(j, cv2.IMREAD_UNCHANGED);
#             temp = cv2.cvtColor(temp, cv2.COLOR_BGRA2RGBA);  # 2
...
#         if image_list[i][0] ==0:
#             temp=crop_near_face(image_list[i][1],clf);
#             image_crop_list.append(temp);
#         else :
#             image_crop_list.append(image_list[i][1]);



#     # 이미지 리사이즈 PART
#     image_resize_list=resize_after_crop(image_crop_list);

#     # 이미지 배치 PART
#     image_map=mapping_after_resize(image_resize_list);

#     #start_position=time.time();

#     index=decision_position(image_map, more=True); # 기본값 : more false, trace false, maxRounds = 50

#     #end_position=time.time();

#     #print('위치 찾기에 걸리는 시간 :', end_position - start_position);

#     # 이미지 배치 PART 결과물 (각이미지가 배경에서 위치해야 할 인덱스(각 이미지의 좌측 상단 시작부분 좌표))
#     w_index = web_index(image_resize_list,index);
#     print(w_index);


#     # 최종 이미지 생성 PART
#     background = np.zeros((600, 400, 3));
#     image_resize_list.append(background);

#     image_padding_list=padding_position(image_resize_list,index);

#     #start_projection=time.time();

#     final_image, final_record=projection_image(image_padding_list);

#     #end_projection = time.time();

#     #print('최종 이미지 생성하는데 걸리는 시간 :', end_projection - start_projection);
